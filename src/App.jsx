  ...
  const calculateRepayment = (event) => {
    ...
    setMonthlyRepayment(() => {
      ...
      if (inputFields.mortgageType === 'repayment') {
        repayment = (amount * rate) / (1 - Math.pow(1 + rate, -term))
        ...
        <div className='w-1/2 pt-10 pl-6'>
          ...
          <div className='pt-12'>
            <form onSubmit={calculateRepayment}>
              ...
              <div>
                <div className={`${inputFields.mortgageType === 'repayment' ? 'h-12 w-full border border-primary-lime rounded-md pt-2 pl-4' : 'h-12 w-full border border-Neutral-slate-500 rounded-md pt-2 pl-4 hover:border-primary-lime'}`} id='radio'>
                  <input type='radio' value='repayment' name='mortgageType' onChange={handleChange} checked={inputFields.mortgageType === 'repayment'} className='w-4 h-4' />
                  <label htmlFor="repayment" className='text-lg text-Neutral-slate-900 font-display font-medium pl-4'>Repayment</label>

                </div>
                <div className={`${inputFields.mortgageType === 'interestOnly' ? 'h-12 w-full border border-primary-lime rounded-md my-2 pt-2 pl-4' : 'h-12 w-full border border-Neutral-slate-500 rounded-md my-2 pt-2 pl-4'}`}>

                  <input type="radio" value='interestOnly' name='mortgageType' onChange={handleChange} checked={inputFields.mortgageType === 'interestOnly'} className='h-4 w-4 text-Neutral-slate-500' id='radio' style={{ border: inputFields.mortgageType === "interestOnly" ? "1px solid red" : "white" }} />
                  <label htmlFor="interest only" className='text-lg text-Neutral-slate-900 font-display font-medium pl-4'>Interest Only</label>
                </div>
                {errors.mortgageType ? (
                  <p className="error text-primary-red">This Field is required!</p>
                ) : null}
              ...
              <div className='px-8 pb-52 pt-10'>
                ...
                <div className='relative bg-primary-lime h-48 rounded-md w-full mt-14'>
                  <div className='absolute bg-Neutral-slate-900 shadow-lg rounded-md  top-[4px] w-full px-4'>
                    ...
                    <h2 className='text-primary-lime text-4xl  font-display'>£ {monthlyRepayment}</h2>
                    ...
                    <p className='pb-8 text-Neutral-white text-lg font-display pt-[4px]'>£ </p>
                  </div>
                </div>
              </div>
            </div>
